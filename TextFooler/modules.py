...
#         x = [F.avg_pool1d(i, i.size(2)).squeeze(2) for i in x] #[(N,Co), ...]
#         x = torch.cat(x, 1)
#         return x

class CNN_Text(nn.Module):

    def __init__(self, n_in, widths=[3,4,5], filters=100, num_classes=2, drop=nn.Dropout):
        super(CNN_Text,self).__init__()
        Ci = 1
        Co = filters
        h = n_in
        self.conv1_si = nn.Conv2d(Ci, Co, (widths[0], h), padding=(2, 0), bias=False)
        self.drop = drop
        self.fc2_si = nn.Linear(Co,  num_classes, bias=False)

    def forward(self, x):
        # x is (batch, len, d)
        x = x.unsqueeze(1) # (batch, Ci, len, d)
        x = F.relu(self.conv1_si(x)).squeeze(3)
...
class EmbeddingLayer(nn.Module):
    def __init__(self, n_d=100, embs=None, fix_emb=True, oov='<oov>', pad='<pad>', normalize=True):
        ...
        self.embedding.weight[-2:].data.zero_()
