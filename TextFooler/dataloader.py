...
def clean_str(string, TREC=False):
    ...
    """
    string = re.sub(r"[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z]+[.]+[a-zA-Z]+[.]?[a-zA-Z]*", 'EMAILID', string)
    string = re.sub(r"https?:\/\/w{0,3}\w*?\.(\w*?\.)?\w{2,3}\S*|www\.(\w*?\.)?\w*?\.\w{2,3}\S*|(\w*?\.)?\w*?\.\w{2,3}[\/\?]\S*", 'WEBADDRESS', string)
    string = re.sub(r"\d{10}\d{0,9}", 'PHONENUMBER', string)
...
def read_corpus(path, clean=True, MR=True, encoding='utf8', shuffle=False, lower=True):
    ...
    data = [text.split() for text in df['text'].values.tolist()]
